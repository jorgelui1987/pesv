...
router.get('/avance', verificarToken, async (req, res) => {
    ...
    const totalPasos = await queryGet('SELECT COUNT(*) as total FROM pasos WHERE empresa_id = ?', [empresaId]);
    const pasosConEvidencia = await queryGet(`
        ...
        WHERE p.empresa_id = ? AND e.estado = 'finalizado'
    `, [empresaId]);
...
router.get('/pdf', verificarToken, async (req, res) => {
    const empresaId = req.usuario.empresa_id;
    const empresa = await queryGet('SELECT * FROM empresas WHERE id = ?', [empresaId]);
    ...
    const totalPasos = await queryGet('SELECT COUNT(*) as t FROM pasos WHERE empresa_id = ?', [empresaId]);
    const completados = await queryGet(`
        SELECT COUNT(DISTINCT p.id) as t FROM pasos p 
        JOIN evidencias e ON e.paso_id = p.id 
        WHERE p.empresa_id = ? AND e.estado = 'finalizado'
    ...
    for (const fase of fases) {
        const pasos = await queryAll('SELECT * FROM pasos WHERE fase_id = ? ORDER BY orden', [fase.id]);
